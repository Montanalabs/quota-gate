...
- **Closed type:** `type Decision = GrantQuota(Quota) | Deny` where `Quota` is closed
- **Sink / capability:** `grant provision` → `using provision { privileged { provision(d) } }`
- **Consequence axes:** Trust, Capabilities

## Run the demo

```sh
examples/apps/quota-gate/demo.sh
```
...
- `quota-gate_safe.wave` — the correct design.
- `quota-gate_unsafe.wave` — the tempting-but-wrong version (the negative example a model must learn to reject).
